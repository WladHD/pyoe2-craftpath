...
static TRANSMUT_ORBS: &[CraftCurrencyEnum] = &[
...
pub struct OrbOfTransmutationPropagator;

impl MatrixPropagator for OrbOfTransmutationPropagator {
    fn propagate_step(
        &self,
        item_instance: &Item,
    ...
    fn is_applicable(&self, item: &Item, _provider: &ItemInfoProvider) -> bool {
        match item.snapshot.rarity {
            ItemRarityEnum::Normal => item.snapshot.affixes.is_empty(),
            _ => false,
        }
    }
}
