...
static AUGMENT_ORBS: &[CraftCurrencyEnum] = &[
...
pub struct OrbOfAugmentationPropagator;

impl MatrixPropagator for OrbOfAugmentationPropagator {
    fn propagate_step(
        &self,
        item_instance: &Item,
        ...
        for currency in AUGMENT_ORBS {
            let force_min_starting_level = ItemLevel::from(match currency {
                ...
                &CraftCurrencyEnum::OrbOfAugmentationGreater() => 55,
            ...
            pool.retain(|affix_id, tier_level_holder| {
                ...
                match affix_def.affix_location {
                    AffixLocationEnum::Prefix => {
                        if item_instance.helper.prefix_count != 0 {
                            return false;
                        }
                    }
                    AffixLocationEnum::Suffix => {
                        if item_instance.helper.suffix_count != 0 {
                            return false;
                        }
                    }
    ...
    fn is_applicable(&self, item: &Item, _provider: &ItemInfoProvider) -> bool {
        match item.snapshot.rarity {
            ItemRarityEnum::Magic => item.snapshot.affixes.len() < 2,
            _ => false,
        }
    }
}
